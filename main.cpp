...
namespace {
    ...
    void TestFormulaArithmetic() {
        ...
        {
            auto value = std::get<double>(ParseFormula(std::move(expr))->Evaluate(*sheet));
            return value;
        };
    ...
    void TestErrorDiv0() {
...

        {
            ...
            auto val = sheet->GetCell("A1"_pos)->GetValue();
            ASSERT_EQUAL(val,
                CellInterface::Value(FormulaError::Category::Div0));
        }
    ...
    void TestClearPrint()
    {
        auto PrintSheet = [](std::unique_ptr<SheetInterface>& sheet)
        {
